// This is brl/vidpro1/process/vidpro1_resample_process.cxx

//:
// \file

#include "vidpro1_resample_process.h"
...
//: Clone the process
bpro1_process* 
vidpro1_resample_process::clone() const
{
  return new vidpro1_resample_process(*this);
}
...
int
vidpro1_resample_process::input_frames()
{
  return 1;
}
...
int
vidpro1_resample_process::output_frames()
{
  return 1;
}
...
bool
vidpro1_resample_process::execute()
{
  ...
  unsigned int ni = (unsigned int)(image_sptr->ni() * scale);
  unsigned int nj = (unsigned int)(image_sptr->nj() * scale);

  vil_image_resource_sptr output_sptr = vil_new_image_resource( ni ,
                                                                nj ,
                                                                image_sptr->nplanes() ,
...
#define macro(F , T) \
     ...
     vil_resample_bilin( vil_image_view<T >(*image_sptr->get_view()) , \
                            output_view , ni, nj ); break; }
...
//: Finish
bool
vidpro1_resample_process::finish()
{
  return true;
}
