...
{
    return vcl_string("var_") + buld_get_random_alphanumeric_string(len - 4);
}

vcl_string getRandomHFileTag(int len)
{
    return vcl_string("H_") + buld_get_random_alphanumeric_string(len - 4) + vcl_string("_H");
}
...
void create_cpp_file(vcl_string& output_cpp_file_name, vcl_string& output_h_file_name, vcl_string& root_path, vcl_vector<vcl_string>& original_files, vcl_vector<vcl_string>& variable_names, vcl_vector<int>& data_sizes, vcl_string& extract_command, vcl_string& delete_command)
{
    ...
    outs << "#include<buld/buld_handle_files_embedded_in_vxl_source.h>" << vcl_endl;
    ...
    outs << "   if(!buld_is_file_extraction_in_progress(dest_dir))" << vcl_endl;
    outs << "   {" << vcl_endl;
    outs << "       if(!vul_file_exists(expanded_path))" << vcl_endl;
    outs << "       {" << vcl_endl;
    outs << "           vul_file::make_directory_path(expanded_path.c_str());" << vcl_endl;
    outs << "           buld_create_file_extraction_lock(dest_dir);" << vcl_endl;
    for(int i = 0; i < original_files.size(); i++)
    {
    outs << "           buld_default_extract_embedded_file(" << variable_names[i] << ", " << data_sizes[i] << ", dest_dir, \"" << original_files[i] << "\");" << vcl_endl;
    }
    outs << "           buld_remove_file_extraction_lock(dest_dir);" << vcl_endl;
    outs << "       }" << vcl_endl;
    outs << "   }" << vcl_endl;
    outs << "   else while(buld_is_file_extraction_in_progress(dest_dir))" << vcl_endl;
    outs << "   {" << vcl_endl;
    ...
    outs << "void " << delete_command << "(const char* dest_dir)" << vcl_endl;
    outs << "{" << vcl_endl;
    outs << "       buld_default_delete_extracted_file(dest_dir);" << vcl_endl;
    outs << "}" << vcl_endl;
