// This is lemsvxlsrc/brcv/shp/dbhg/dbgrl_hg_hypergraph.h
//:
// \file
...
class dbgrl_hg_hypergraph : public dbgrl_hg_graph
{
protected:
  vcl_map<int, dbgrl_hg_hyperedge*> hyperedges_;

public:
  //: ====== Hypergraph query functions ======
  vcl_map<int, dbgrl_hg_hyperedge*>& hyperedges() {
    return hyperedges_;
  }
  dbgrl_hg_hyperedge* hyperedges (const int i) {
    vcl_map<int, dbgrl_hg_hyperedge*>::iterator H_it = hyperedges_.find (i);
    if (H_it == hyperedges_.end())
      return NULL;
    
    dbgrl_hg_hyperedge* hyperedge = (*H_it).second;
    return hyperedge;
  }
  ...
  void _internal_add_hyperedge (dbgrl_hg_hyperedge* hyperedge);
  bool _internal_del_hyperedge (dbgrl_hg_hyperedge* hyperedge);

  //: ===== Internal Mid-level Graph operation (handling connectivity) =====
  void _internal_connect_hyperedge_edge (dbgrl_hg_hyperedge* hyperedge, dbgrl_hg_edge* edge) {
    //: unordered edge list
    hyperedge->connect_edge (edge);
    edge->connect_hyperedge (hyperedge);
  }
  void _internal_disconnect_hyperedge_edge (dbgrl_hg_hyperedge* hyperedge, dbgrl_hg_edge* edge) {
    //: unordered edge list
    hyperedge->disconnect_edge (edge);
    edge->disconnect_hyperedge (hyperedge);
  }

  void _internal_connect_hyperedge_vertex (dbgrl_hg_hyperedge* hyperedge, dbgrl_hg_vertex* vertex) {
    hyperedge->connect_vertex (vertex);
    vertex->connect_hyperedge (hyperedge);
  }
  void _internal_disconnect_hyperedge_vertex (dbgrl_hg_hyperedge* hyperedge, dbgrl_hg_vertex* vertex) {
  ...
  virtual bool remove_hyperedge (dbgrl_hg_hyperedge* hyperedge);
  ...
  virtual bool topo_remove_hyperedge (dbgrl_hg_hyperedge* hyperedge);
