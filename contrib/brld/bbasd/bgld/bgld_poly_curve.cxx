// This is basic/bgld/bgld_poly_curve.cxx
...
// then bgld_poly_curve won't be an abstract class anymore
//virtual double length() const = 0;  

//: Compute the point at arclength s using polynomials 
vgl_point_2d<double> bgld_poly_curve::point_at(double s) const 
{
  return 
...
vgl_vector_2d<double> bgld_poly_curve::tangent_at(double s) const
{
...
double bgld_poly_curve::tangent_angle_at(double s) const 
{
...
//: evaluate at s
double bgld_poly_curve::evaluate_x(double s) const 
{
  double sum=0;
...
double bgld_poly_curve::evaluate_y(double s) const
{
  double sum=0;
  for (int i = 0; i<order_+1; i++) 
...
double bgld_poly_curve::evaluate_derivative_x(double s) const
{
  double sum=0;
...
double bgld_poly_curve::evaluate_derivative_y(double s) const
{
  double sum=0;
  for (int i = 1; i<order_+1; i++) 
...
bool bgld_poly_curve::operator==(bgld_poly_curve const& c) const 
{
...
bgld_poly_curve& bgld_poly_curve::operator=( bgld_poly_curve const& c) 
{
  order_ = c.order();
  len_ = c.length();
