// This is bbasd/bnld/algo/bnld_bbf_tree.h
#ifndef bnld_bbf_tree_h_
#define bnld_bbf_tree_h_
//:
// \file
...
public:
  //: Constructor
  bnld_bbf_box<T,n>();
  bnld_bbf_box<T,n>( const vnl_vector_fixed<T,n>& min_point,
                const vnl_vector_fixed<T,n>& max_point );

  //: Copy constructor
  bnld_bbf_box<T,n>( const bnld_bbf_box<T,n>& old );
...
template <class T, unsigned int n>
class bnld_bbf_node : public vbl_ref_count
{
 public:
  ...
  //: Outer bounding box
  bnld_bbf_box<T,n> outer_box_;
  //: Inner bounding box
  bnld_bbf_box<T,n> inner_box_;
  ...
  //: Right child
  bnld_bbf_node_sptr right_;
};


// A light-weight class used in the priorty queue
template <class T, unsigned int n>
class bnld_bbf_queue_entry
{
public:
  typedef vbl_smart_ptr<bnld_bbf_node< T,n > > bnld_bbf_node_sptr;

  //: Constructor
  bnld_bbf_queue_entry() {}
  //: Constructor
  bnld_bbf_queue_entry( T dist, bnld_bbf_node_sptr node )
    : dist_(dist), node_(node) {}
  //: Used in sorting by distance
  bool operator< ( const bnld_bbf_queue_entry& right ) const
  { return right.dist_ < this->dist_; } 

  //: Distance to this node
  T dist_;
  //: Smart pointer to the node
  bnld_bbf_node_sptr node_;
};
