...
interpolate(double const data[BNLD_ENO_DATA_LENGTH], double const x[BNLD_ENO_DATA_LENGTH])
{
  ...
  coeffs_[bnld_eno_poly::second_order_index] = a2;

  a2 = -a2;
  d21 = x[2]-x[1];
  coeffs_[bnld_eno_poly::first_order_index] = 
     (data[2] - data[1])/d21 + a2*(x[1] + x[2]);

  coeffs_[bnld_eno_poly::zero_order_index] =
     data[1] + x[1]*(a2*x[1] - coeffs_[bnld_eno_poly::first_order_index]);
...
interpolate(double d1, double d2, double d3, double x1, double x2, double x3)
{
  ...
  coeffs_[bnld_eno_poly::zero_order_index] =
     d1 + x1*(a2*x1 - coeffs_[bnld_eno_poly::first_order_index]);
...
interpolate(double const data[BNLD_ENO_DATA_LENGTH])
{
  ...
  /* compute and store all polynomial coefficients for this interpolant */
  coeffs_[bnld_eno_poly::second_order_index] = c2;
  coeffs_[bnld_eno_poly::first_order_index] = data[off+1] - (c2*(2*start_+1) + data[off+0]);
  coeffs_[bnld_eno_poly::zero_order_index] =
