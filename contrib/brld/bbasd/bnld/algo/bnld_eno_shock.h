...
public:
   ...
   const bnld_eno_interp &next() const {return *next_;}
   const bnld_eno_interp &prev() const {return *prev_;}
   //: makes the shock
   unsigned make(bnld_eno_interp const *prev, bnld_eno_interp const *curr,
...
private:
   ...
   bnld_eno_interp const *next_, *prev_;
   double loc_;
   bool no_intercept_;
};
...
public:
   ...
   bool test_shock(unsigned i, 
         double tt=bnld_eno_shock_1d::default_tangent_change_threshold,
         double ct=bnld_eno_shock_1d::default_curvature_change_threshold) const;
   ...
   bool place_shocks(
         double tt=bnld_eno_shock_1d::default_tangent_change_threshold,
         double ct=bnld_eno_shock_1d::default_curvature_change_threshold);

   //: make sure has_shock(i) is true before using this
   const bnld_eno_shock_interp &shock(unsigned i) const {return *(s_[i]);}
   const bnld_eno_measures& measures(unsigned i) const {return m_[i];}
   ...
   // bnld_eno_interp because I wanted that class to be totally independent of shock
   // placing concepts.
   // \relates bnld_eno_interp
   ...
   // eno_shock_1d. 
   static bool test(
         bnld_eno_interp const   *prev,
         bnld_eno_interp const   *curr, 
         bnld_eno_interp const   *next,
   ...
   // eno_shock_1d. 
   static bool test(
         bnld_eno_interp const   *prev, bnld_eno_measures const *prevm,
         bnld_eno_interp const   *curr, bnld_eno_measures const *currm,
         bnld_eno_interp const   *next, bnld_eno_measures const *nextm,
         double shock_tangent_change_threshold=default_tangent_change_threshold, 
         double shock_curvature_change_threshold=default_curvature_change_threshold
      );
