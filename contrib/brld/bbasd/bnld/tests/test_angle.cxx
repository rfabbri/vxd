...
MAIN( test_angle )
{
  ...
  TEST("angle_0to2pi - in ranage", bnld_angle_0to2pi(1.7), 1.7);
  TEST_NEAR("angle_0to2pi - below", bnld_angle_0to2pi(-7.1), -7.1 + 4*vnl_math::pi, 1e-8);
  TEST_NEAR("angle_0to2pi - above", bnld_angle_0to2pi(7.1), 7.1 - 2*vnl_math::pi, 1e-8);

  double twopi = vnl_math::pi * 2;
  double y[] = {6.0, 6.01, 6.5, 6.8};
  double x[] = {y[2]-twopi, y[3] + 5*twopi, y[0], y[1]-4*twopi};
  unsigned num_pts = 4;
  bnld_angle_sort_ascending(x, num_pts);
