// This is dbsol_gshock_curve_2d.h
#ifndef dbsol_gshock_curve_2d_h
#define dbsol_gshock_curve_2d_h
//:
//\file
...
#include <dbsol/dbsol_gshock_sptr.h>
...
class dbsol_gshock_curve_2d : public dbsol_geno_curve_2d {
 public:
   dbsol_gshock_curve_2d() :
      dbsol_geno_curve_2d(),
      nshocks_(0) { }
   ...
   //: return cached dbsol_geno_measures for i-th interval
   const dbsol_geno_measures & measure(unsigned i) const { return m_[i]; }

   //: If i-th interval has an associated shock object, 
   // return smart pointer to the shock object. Otherwise, return null
   const dbsol_gshock_sptr shock(unsigned i) const {return s_[i];}
 ...
 protected:
   ...
   virtual void make(const vcl_vector<dbgl_param_curve *> &inter, bool closed) 
      { dbsol_geno_curve_2d::make(inter,closed); m_.clear(); cache_measures(); 
   ...
   vcl_vector<dbsol_gshock_sptr> s_;
 private:
   //: cache of tangent and curvature measurements of intervals
   vcl_vector<dbsol_geno_measures> m_;
   //: shock mark:
   vcl_vector<bool> skmark_;
   unsigned nshocks_;
};

#endif // dbsol_gshock_curve_2d_h
