...
#include <bpro1d/bpro1d_process_sptr.h>
...
public:
  ...
  bpro1d_process_sptr get_process_by_name( const vcl_string& name ) const;
  ...
  bool run_process_on_current_frame( const bpro1d_process_sptr& process,
                                     vcl_set<bpro1d_storage_sptr>* modified = NULL );
  bool run_process_queue_on_current_frame ( vcl_set<bpro1d_storage_sptr>* modified = NULL );

  //: Call the finish function on the process 
  // \param modified is an optional parameter.  If specified, all storage objects
  //                 modified by the process are added to the set
  bool finish_process( int first_frame, int last_frame,
                       const bpro1d_process_sptr& process_block,
                       vcl_set<bpro1d_storage_sptr>* modified = NULL );
  ...
  void add_process_to_queue(const bpro1d_process_sptr& process);
  void delete_last_process();
  void clear_process_queue();

  vcl_vector <vcl_string> get_process_queue_list() const;

  static void register_process( const bpro1d_process_sptr& sptr );
  //: to access frames from future alse
  void set_non_causual(bool non_causal);
private:
  vidpro1_repository_sptr repository_sptr;

  static vcl_multimap< vcl_string , bpro1d_process_sptr > process_map;
  vcl_vector< bpro1d_process_sptr > process_queue;
